<x-app-layout>
    <x-slot name="title">{{ __('Dana Keluar Eksternal') }}</x-slot>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-slate-800 leading-tight">{{ __('Daftar Pengeluaran Eksternal') }}</h2>
    </x-slot>

    <div class="max-w-7xl mx-auto">
        <div class="card-modern overflow-hidden mb-6">
            <div class="p-4 border-b border-gray-100">
                <form method="GET" action="{{ route('cash-flows.out.external.index') }}" class="flex flex-wrap gap-4 items-end">
                    @if(($canFilterLocation ?? false) || ($filterLocked ?? false))
                        ...
                        <div class="min-w-[140px]">
                            ...
                            <select name="location_type" id="out_location_type" class="w-full rounded-lg border border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 {{ $branchSelectDisabled ? 'bg-slate-100 cursor-not-allowed' : '' }}" {{ $branchSelectDisabled ? 'disabled' : '' }}>
                        ...
                        <div id="out_location_wrapper" class="min-w-[180px]" style="{{ $locType === '' ? 'display:none' : '' }}">
                            <label class="block text-sm font-medium text-slate-700 mb-1" id="out_location_label">{{ $locType === 'warehouse' ? __('Gudang') : __('Cabang') }}</label>
                            <div class="filter-out-warehouse" style="{{ $locType !== 'warehouse' ? 'display:none' : '' }}">
                            ...
                            <div class="filter-out-branch" style="{{ $locType !== 'branch' ? 'display:none' : '' }}">
                    ...
                    <div class="flex gap-2">
                        ...
                        <a href="{{ route('cash-flows.out.external.index') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-slate-100 text-slate-700 text-sm font-medium hover:bg-slate-200">
                            {{ __('Reset') }}
                        </a>
                        <a href="{{ route('cash-flows.out.external.create') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-red-600 text-white text-sm font-medium hover:bg-red-700">
                            {{ __('Catat Pengeluaran Eksternal') }}
                        </a>
                    </div>
                </form>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <div class="card-modern p-6">
                <p class="text-sm text-slate-600">{{ __('Total Pengeluaran Eksternal') }}</p>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    @forelse ($expenses as $exp)
                        <tr class="hover:bg-slate-50/50">
                            ...
                            <td class="px-4 py-3">
                                @if ($exp->warehouse_id)
                                    {{ __('Gudang') }}: {{ $exp->warehouse?->name ?? '-' }}
                                @else
                                    {{ __('Cabang') }}: {{ $exp->branch?->name ?? '-' }}
                                @endif
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-4 py-12 text-center text-slate-500">{{ __('Tidak ada data pengeluaran eksternal.') }}</td>
                        </tr>
                    @endforelse
                    </tbody>
                </table>
        ...
        @push('scripts')
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    const locType = document.getElementById('out_location_type');
                    const wrapper = document.getElementById('out_location_wrapper');
                    const label = document.getElementById('out_location_label');
                    const whBlock = document.querySelector('.filter-out-warehouse');
                    const brBlock = document.querySelector('.filter-out-branch');
