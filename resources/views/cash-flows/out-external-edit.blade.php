<x-app-layout>
    <x-slot name="title">{{ __('Edit Pengeluaran Dana Eksternal') }} #{{ $cashFlow->id }}</x-slot>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-slate-800 leading-tight">
                {{ __('Edit Pengeluaran Dana Eksternal') }} #{{ $cashFlow->id }}
            </h2>
    ...
    <div class="max-w-3xl mx-auto">
        ...
        <div class="card-modern overflow-hidden">
            <div class="p-6">
                <form method="POST" action="{{ route('cash-flows.out.external.update', $cashFlow) }}" class="space-y-5">
                    @csrf
                    @method('PUT')
