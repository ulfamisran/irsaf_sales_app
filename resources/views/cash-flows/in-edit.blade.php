<x-app-layout>
    <x-slot name="title">{{ __('Edit Pemasukan Lainnya') }} #{{ $cashFlow->id }}</x-slot>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-slate-800 leading-tight">
                {{ __('Edit Pemasukan Lainnya') }} #{{ $cashFlow->id }}
            </h2>
    ...
    <div class="max-w-3xl mx-auto">
        ...
        <div class="card-modern overflow-hidden">
            <div class="p-6">
                <form method="POST" action="{{ route('cash-flows.in.update', $cashFlow) }}" class="space-y-5">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <p class="text-sm text-slate-500">{{ __('Kategori') }}</p>
                            <p class="font-medium text-slate-800">{{ $cashFlow->incomeCategory?->name ?? '-' }}</p>
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        <x-text-input id="description" class="block mt-1 w-full" type="text" name="description" :value="old('description', $cashFlow->description ?? '')" />
                    ...
                    <div class="flex gap-3 pt-2">
                        ...
                        <a href="{{ route('cash-flows.in.index') }}" class="inline-flex items-center px-4 py-2.5 bg-gray-200 border border-transparent rounded-lg font-semibold text-sm text-gray-700 hover:bg-gray-300">
                            {{ __('Batal') }}
                        </a>
                    </div>
                </form>
