<x-app-layout>
                ...
                <form method="GET" action="{{ route('finance.cash-monitoring') }}" class="flex flex-wrap gap-4 items-end">
                    @if(($canFilterLocation ?? false) || ($filterLocked ?? false))
                        ...
                        <div class="min-w-[180px]">
                            ...
                            <select name="location_type" id="cm_location_type" class="w-full rounded-lg border border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm {{ $branchSelectDisabled ? 'bg-slate-100 cursor-not-allowed' : '' }}" {{ $branchSelectDisabled ? 'disabled' : '' }}>
                        ...
                        <div id="cm_location_wrapper" class="min-w-[180px]" style="{{ $locType === '' ? 'display:none' : '' }}">
                            <label class="block text-sm font-medium text-slate-700 mb-1" id="cm_location_label">{{ $locType === 'warehouse' ? __('Gudang') : __('Cabang') }}</label>
                            <div class="filter-cm-warehouse" style="{{ $locType !== 'warehouse' ? 'display:none' : '' }}">
                            ...
                            <div class="filter-cm-branch" style="{{ $locType !== 'branch' ? 'display:none' : '' }}">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const locType = document.getElementById('cm_location_type');
            const wrapper = document.getElementById('cm_location_wrapper');
            const label = document.getElementById('cm_location_label');
            const whBlock = document.querySelector('.filter-cm-warehouse');
            const brBlock = document.querySelector('.filter-cm-branch');
