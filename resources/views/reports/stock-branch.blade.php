<x-app-layout>
    <x-slot name="title">{{ __('Laporan Stok Cabang') }}</x-slot>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-slate-800 leading-tight">{{ __('Laporan Stok per Cabang') }}</h2>
    ...
    <div class="max-w-7xl mx-auto">
        <div class="card-modern overflow-hidden mb-6">
            <div class="p-4 border-b border-gray-100">
                <form method="GET" action="{{ route('reports.stock-branch') }}" class="flex flex-wrap gap-4 items-end">
                    ...
                    <div class="flex gap-2">
                        ...
                        <a href="{{ route('reports.stock-branch') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-slate-100 text-slate-700 text-sm font-medium hover:bg-slate-200">
                            {{ __('Reset') }}
                        </a>
                    </div>
                </form>
