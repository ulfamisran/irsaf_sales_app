<x-app-layout>
    <x-slot name="title">{{ __('Edit Gudang') }}</x-slot>
    <x-slot name="header">
                    ...
                    <form method="POST" action="{{ route('warehouses.update', $warehouse) }}">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <textarea id="address" name="address" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" rows="3">{{ old('address', $warehouse->address) }}</textarea>
                                <x-input-error :messages="$errors->get('address')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="pic_name" :value="__('PIC Name')" />
                                <x-text-input id="pic_name" class="block mt-1 w-full" type="text" name="pic_name" :value="old('pic_name', $warehouse->pic_name)" />
