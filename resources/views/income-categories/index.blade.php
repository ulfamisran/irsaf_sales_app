<x-app-layout>
    <x-slot name="title">{{ __('Kategori Pemasukan') }}</x-slot>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-slate-800 leading-tight">{{ __('Kategori Pemasukan') }}</h2>
            @if (auth()->user()->isSuperAdmin())
                <x-icon-btn-add :href="route('income-categories.create')" :label="__('Tambah Kategori Pemasukan')" />
            @else
    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="card-modern overflow-hidden mb-6">
            <div class="p-4 border-b border-gray-100">
                <form method="GET" action="{{ route('income-categories.index') }}" class="flex flex-wrap gap-3 items-end">
                    ...
                    <div class="flex gap-2">
                        ...
                        <a href="{{ route('income-categories.index') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-slate-100 text-slate-700 text-sm font-medium hover:bg-slate-200">
                            {{ __('Reset') }}
                        </a>
                    </div>
                </form>
                        ...
                        <tr class="hover:bg-slate-50/50">
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    @if (auth()->user()->isSuperAdmin())
                                        <x-icon-btn-edit :href="route('income-categories.edit', $category)" />
