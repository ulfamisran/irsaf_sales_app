<x-app-layout>
    <x-slot name="title">{{ __('Edit Metode Pembayaran') }}</x-slot>
    <x-slot name="header">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <x-text-input id="nama_bank" class="block mt-1 w-full" type="text" name="nama_bank" :value="old('nama_bank', $paymentMethod->nama_bank)" />
                                    <x-input-error :messages="$errors->get('nama_bank')" class="mt-2" />
                                </div>
                                <div>
                                    <x-input-label for="no_rekening" :value="__('No Rekening (Opsional)')" />
                                    <x-text-input id="no_rekening" class="block mt-1 w-full" type="text" name="no_rekening" :value="old('no_rekening', $paymentMethod->no_rekening)" />
                            ...
                            <div>
                                ...
                                <x-text-input id="atas_nama_bank" class="block mt-1 w-full" type="text" name="atas_nama_bank" :value="old('atas_nama_bank', $paymentMethod->atas_nama_bank)" />
                                <x-input-error :messages="$errors->get('atas_nama_bank')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="keterangan" :value="__('Keterangan (Opsional)')" />
                                <textarea id="keterangan" name="keterangan" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" rows="3">{{ old('keterangan', $paymentMethod->keterangan) }}</textarea>
                            ...
                            <div class="flex items-center gap-2">
                                <input id="is_active" name="is_active" type="checkbox" value="1" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                                    {{ old('is_active', $paymentMethod->is_active ? '1' : '0') === '1' ? 'checked' : '' }}>
