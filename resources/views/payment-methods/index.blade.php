<x-app-layout>
    <x-slot name="title">{{ __('Metode Pembayaran') }}</x-slot>
    <x-slot name="header">
    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="card-modern overflow-hidden mb-6">
            <div class="p-4 border-b border-gray-100">
                <form method="GET" action="{{ route('payment-methods.index') }}" class="flex flex-wrap gap-3 items-end">
                    ...
                    <div class="flex gap-2">
                        ...
                        <a href="{{ route('payment-methods.index') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-slate-100 text-slate-700 text-sm font-medium hover:bg-slate-200">
                            {{ __('Reset') }}
                        </a>
                    </div>
                </form>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead>
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">{{ __('No Rekening') }}</th>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse ($methods as $m)
                            <tr class="hover:bg-slate-50/50">
                                ...
                                <td class="px-4 py-3 font-mono text-sm">{{ $m->no_rekening ?? '-' }}</td>
                                <td class="px-4 py-3">
                                    <span class="px-2 py-1 rounded-lg text-xs font-medium {{ $m->is_active ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-700' }}">
                                        {{ $m->is_active ? __('Aktif') : __('Nonaktif') }}
                                    </span>
                                </td>
