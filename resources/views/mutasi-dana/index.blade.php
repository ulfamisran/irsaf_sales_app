<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-slate-800 leading-tight">{{ __('Riwayat Mutasi Dana') }}</h2>
            <x-icon-btn-add :href="route('mutasi-dana.create')" :label="__('Mutasi Dana Baru')" />
        </div>
    </x-slot>
    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="card-modern overflow-hidden mb-6">
            <div class="p-4 border-b border-gray-100">
                <form method="GET" action="{{ route('mutasi-dana.index') }}" class="flex flex-wrap gap-3 items-end">
                    @if ($canFilterLocation)
                    <div class="min-w-[140px]">
                        <label class="block text-sm font-medium text-slate-700 mb-1">{{ __('Tipe Lokasi') }}</label>
                        <select name="location_type" id="filter_location_type" class="w-full rounded-lg border border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                    ...
                    <div class="min-w-[180px]">
                        ...
                        <select name="location_id" class="w-full rounded-lg border border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                            ...
                            @foreach ($warehouses as $w)
                                <option value="{{ $w->id }}" data-type="warehouse" {{ request('location_type') === 'warehouse' && request('location_id') == $w->id ? 'selected' : '' }}>{{ __('Gudang') }}: {{ $w->name }}</option>
                            @endforeach
                            @foreach ($branches as $b)
                                <option value="{{ $b->id }}" data-type="branch" {{ request('location_type') === 'branch' && request('location_id') == $b->id ? 'selected' : '' }}>{{ __('Cabang') }}: {{ $b->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    @elseif ($filterLocked)
                    ...
                    <div class="flex gap-2">
                        ...
                        <a href="{{ route('mutasi-dana.index') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-slate-100 text-slate-700 text-sm font-medium hover:bg-slate-200">{{ __('Reset') }}</a>
                    </div>
                </form>
            </div>
        </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead>
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">{{ __('Dana Asal') }}</th>
